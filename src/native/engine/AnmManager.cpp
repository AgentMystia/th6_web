...
ZunResult AnmManager::LoadTexture(i32 textureIdx, char *textureName, i32 textureFormat, D3DCOLOR colorKey)
{
    ...
    if (this->imageDataArray[textureIdx] == NULL)
    {
        ...
        fprintf(stderr, "[anm] FAIL load tex %d '%s'\n", textureIdx, textureName ? textureName : "?");
#endif
        return ZUN_ERROR;
    }
#ifdef TH06_BOOT_TRACE
    ...
    if (texLoadCount++ < 40)
        fprintf(stderr, "[anm] load tex %d '%s' size=%u fmt=%d ck=%08X\n", textureIdx, textureName ? textureName : "?",
                th06::g_LastFileSize, textureFormat, (unsigned)colorKey);
#endif
...
ZunResult AnmManager::DrawInner(AnmVm *vm, i32 param_3)
{
    if (param_3 != 0)
    {
        // WASM/clang port of the original x87 inline asm: snap the quad's
        // left/right/top/bottom edges to round(coord) - 0.5 for pixel-center
        // alignment. rintf() uses round-half-to-even, matching x87 `frndint`.
        // Read all four corners first, then write (matches the original order).
        f32 left = rintf(g_PrimitivesToDrawVertexBuf[0].position.x) - g_ZeroPointFive;
        f32 right = rintf(g_PrimitivesToDrawVertexBuf[1].position.x) - g_ZeroPointFive;
        f32 top = rintf(g_PrimitivesToDrawVertexBuf[0].position.y) - g_ZeroPointFive;
        f32 bottom = rintf(g_PrimitivesToDrawVertexBuf[2].position.y) - g_ZeroPointFive;
        g_PrimitivesToDrawVertexBuf[2].position.y = g_PrimitivesToDrawVertexBuf[3].position.y = bottom;
        g_PrimitivesToDrawVertexBuf[0].position.y = g_PrimitivesToDrawVertexBuf[1].position.y = top;
        g_PrimitivesToDrawVertexBuf[1].position.x = g_PrimitivesToDrawVertexBuf[3].position.x = right;
        g_PrimitivesToDrawVertexBuf[0].position.x = g_PrimitivesToDrawVertexBuf[2].position.x = left;
    }
...
ZunResult AnmManager::Draw(AnmVm *vm)
{
    ...
    if (drawFrameCount++ % (120 * 40) == 0) // rough: 40 Draw calls per frame × 120 frames
        fprintf(stderr, "[anm] Draw called %d times in last period\n", drawCallCount);
#endif
    f32 zSine;
    f32 zCosine;
    f32 spriteXCenter;
...
ZunResult AnmManager::Draw3(AnmVm *vm)
{
    ...
    // GPU 3D path: submit model-space XYZ vertices so the shader performs
    // perspective-correct texture interpolation (fixes lake diagonal seams).
    // Fog is also applied per-pixel by the shader instead of per-vertex on CPU.
    D3DXVECTOR3 corners[4] = {
        D3DXVECTOR3(-128, -128, 0), D3DXVECTOR3(128, -128, 0),
        D3DXVECTOR3(-128,  128, 0), D3DXVECTOR3(128,  128, 0)
    };

    // Visibility culling: project corners and check screen bounds.
    {
        D3DXVECTOR3 s;
        D3DXVec3Project(&s, &corners[0], &g_Supervisor.viewport,
                        &g_Supervisor.projectionMatrix, &g_Supervisor.viewMatrix,
                        &worldTransformMatrix);
        float minX = s.x, maxX = s.x, minY = s.y, maxY = s.y;
        for (int i = 1; i < 4; i++)
        {
    ...
    // Build XYZ vertices with model-space positions; fog handled by shader.
    VertexTex1DiffuseXyz verts[4];
    verts[0].position = D3DXVECTOR3(-128, -128, 0);
    verts[1].position = D3DXVECTOR3(128, -128, 0);
    verts[2].position = D3DXVECTOR3(-128, 128, 0);
    verts[3].position = D3DXVECTOR3(128, 128, 0);
    verts[0].diffuse = verts[1].diffuse = verts[2].diffuse = verts[3].diffuse = vm->color;
    verts[0].textureUV = D3DXVECTOR2(vm->sprite->uvStart.x + vm->uvScrollPos.x,
                                     vm->sprite->uvStart.y + vm->uvScrollPos.y);
    verts[1].textureUV = D3DXVECTOR2(vm->sprite->uvEnd.x + vm->uvScrollPos.x,
                                     vm->sprite->uvStart.y + vm->uvScrollPos.y);
    verts[2].textureUV = D3DXVECTOR2(vm->sprite->uvStart.x + vm->uvScrollPos.x,
                                     vm->sprite->uvEnd.y + vm->uvScrollPos.y);
    verts[3].textureUV = D3DXVECTOR2(vm->sprite->uvEnd.x + vm->uvScrollPos.x,
                                     vm->sprite->uvEnd.y + vm->uvScrollPos.y);
    ...
    this->currentVertexShader = 2;
    this->SetRenderStateForVm(vm);
    g_Supervisor.d3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, verts, sizeof(VertexTex1DiffuseXyz));
    D3DXMATRIX restoreWorld;
    D3DXMatrixIdentity(&restoreWorld);
    g_Supervisor.d3dDevice->SetTransform(D3DTS_WORLD, &restoreWorld);
    return ZUN_SUCCESS;
}
